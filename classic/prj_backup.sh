...
# yaml files
pipelines=$(mktemp --tmpdir=. -t .pipXXX).json   # all pipelines ? 
...
codefresh get pip --project $SRC_PRJ --limit --all -o json | jq . > $pipelines
...
do
  ...
  yq -P eval '.[] | select(.metadata.id==env(id)) 
                  | with(.metadata; 
                      ...
                      del(.project,.id)) | .' - > $pipeline

  codefresh create pipeline -f $pipeline
done
rm $pipeline $pipelines 
